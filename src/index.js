...
let cntrCell = document.querySelector("td");
/*************************************************************************** */

let startQuestion = window.confirm("Let's start our game!");

if (startQuestion ){
    console.log("start")
for (let questionId in questions) {
    if (askQuestion(questions[questionId])) {
        console.log(questionId)
        let picpart = pictureParts.filter((part) => part.id == Number(questionId) );
        console.log(picpart)
        colorPicture(picpart);
        cntrCell.style.background = "green";
    }
    else {
        console.log("wrong")
        cntrCell.style.background = "red";
    }
...
function colorPicture (picPart){
    let table = document.querySelector("table");
     for (const point of picPart[0].points) {
         table.childNodes[point[0]].childNodes[point[1]].style.background = picPart[0].color;
    }
}